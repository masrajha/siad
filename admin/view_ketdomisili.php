          ...
          <table class="table table-striped table-bordered table-hover" id="dataTables-example10">
              ...
              <tbody>
                  ...
                  if($_SESSION['level']=="admin" || $_SESSION['level']=="lurah"){
                    $q=mysqli_query($connect, "select * from tbl_ketdomisili");
                  }else{
                    $rt = $_SESSION['rt'];
                    $q=mysqli_query($connect, "select * from tbl_ketdomisili where id_rt='$rt'");
                  }
                  ...
                  <tr >
                      ...
                      <?php
                         if($_SESSION['level']=='admin'){
                      ?>
                       <a href="aksi/delete_ketdomisili.php?id=<?php echo $data['id_domisili']; ?>" class="btn btn-danger"><i class="fa fa-trash"></i> Delete</a>
                        <?php
                      }
                        if($data['verif_rt']=="N" && $_SESSION['level']=='rt'){
                         ?>
                        <a href="aksi/verifikasi_ketdomisili.php?id=<?php echo $data['id_domisili']; ?>&ket=rt" class="btn btn-info" title="Verifikasi"><i class="fa fa-sign-out"></i></a>
                        <?php
                      }else if($data['verif_admin']=="N" && $_SESSION['level']=='admin'){
                         ?>
                        <a href="aksi/verifikasi_ketdomisili.php?id=<?php echo $data['id_domisili']; ?>&ket=admin" class="btn btn-info" title="Verifikasi"><i class="fa fa-sign-out"></i></a>
                        <?php
                      }else if($data['verif_lurah']=="N" && $data['verif_admin']=="Y" && $_SESSION['level']=='lurah'){
                         ?>
                        <a href="aksi/verifikasi_ketdomisili.php?id=<?php echo $data['id_domisili']; ?>&ket=lurah" class="btn btn-info" title="Verifikasi"><i class="fa fa-sign-out"></i></a>
                        <?php
                      } ?>
                      </td>
